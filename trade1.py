...
print("Login OK")
post_message(myToken,"#coin", "Autotrade start XRP. Good luck!")
    ...
    if start_time < now < end_time:
        ...
        while i < 3:
            ...
            if i==0 and (target_price-50) <= current_price < (target_price+100):
                upbit.buy_market_order(ticker, total*rate60)
                time.sleep(1)
            ...
            if i==1 and current_price < buy_average*rate_minus:
                upbit.buy_market_order(ticker, total*rate20)
                time.sleep(1)
                buy_average = get_buy_average(currency)
                i += 1
                print("%dnd Buy OK" % (i))
            ...
            # 매수 3차
            if i==2 and current_price => buy_average*rate_plus:
                upbit.buy_market_order(ticker, total*rate20)
                time.sleep(1)
                buy_average = get_buy_average(currency)
                i += 1
